...
const EditProfileModal = forwardRef(({ handlerEditProfileClick, handlerUserDropdownClick }, ref) => {
	...
	return (
		<div className={css.edit_container} ref={ref}>
			<button onClick={handlerEditProfileClick} className={css.close_button}>
				<svg className={css.close_icon}>
					<use href={icons + "#close"}></use>
				</svg>
			</button>
			<Formik initialValues={{ avatarURL: "", name: `${userName}` }} enableReinitialize={true} onSubmit={userInfoFormSubmit}>
				<Form>
					<div className={css.icon_container}>
						<img src={avatarURL || defaultUserImage} alt="User photo" id="user_image" className={css.user_large_icon} />
						<Field id="file_upload" type="file" name="avatarURL" onChange={onImageChange} />
						<label htmlFor="file_upload">
							<svg className={css.plus_icon} viewBox="-5 -5 70 100">
					...
					<MainButton disabled={!isButtonEnabled} propClass={buttonCss.largeButton} title="Save changes" />
				</Form>
			</Formik>
		</div>
	);
